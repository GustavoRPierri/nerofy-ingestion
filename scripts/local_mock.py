...
_OUTPUT_DIR = (
    Path("/tmp/local_output") if os.environ.get("AWS_SAM_LOCAL") else _ROOT / "local_output"
)
...
class MockSyncRepository(ITransactionSyncRepository):
    ...
    async def save(self, record: TransactionSyncRecord) -> None:
        ...
        _log.info(
            "[MockSyncRepo] cursor salvo: account=%s last_synced=%s total=%d",
            record.account_id,
...
class MockS3Adapter:
    ...
    async def save_json(self, base_path: str, filename: str, data: dict) -> None:
        ...
        dest.write_text(
            json.dumps(data, default=str, indent=2, ensure_ascii=False), encoding="utf-8"
        )
